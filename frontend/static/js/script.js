document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayResults(data) {
        document.getElementById('extracted-text').textContent = data.extracted_text || 'No text was extracted.';
        ...
        data.factors.forEach(factor => {
            ...
            factorEl.className = 'inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2';
            factorEl.textContent = `${factor.factor} (${(factor.confidence*100).toFixed(0)}%)`;
        ...
        recsList.className = 'list-disc list-inside space-y-2';
    ...
    function getRiskClass(level) {
        switch (level.toLowerCase()) {
            ...
            case 'moderate': return { bgColor: 'bg-yellow-100', textColor: 'text-yellow-800' };
